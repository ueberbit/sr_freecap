...
class ImageGeneratorController extends ActionController
{
    ...
    protected function processSettings()
    {
        ...
        for ($i = 0; $i < count($this->settings['fontLocations']); $i++) {
        ...
        $this->settings['imageWidth'] = ($this->settings['maxWordLength'] * (array_sum($this->settings['fontWidths'])/count($this->settings['fontWidths']))) + (isset($this->settings['imageAdditionalWidth']) ? intval($this->settings['imageAdditionalWidth']) : 40);
