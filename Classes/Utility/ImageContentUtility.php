...
class ImageContentUtility
{
    ...
    public static function writeWordOnImage($width, $height, $word, $textColorType, $textPosition, $colorMaximum, $backgroundType, $fontLocations, $fontWidths, $morphFactor = 1)
    {
        ...
        for ($i = 0; $i < strlen($word); $i++) {
            ...
            $fontIndex = RandomContentUtility::getRandomNumberInRange(0, count($fontLocations)-1);
     ...
     * @param bool $morphBackground: if TRUE, the background will be morphed
     * @param bool $blurBackground: if TRUE, the background will be blurred
     * @return string GD image identifier of noisy background
     */
    public static function generateNoisyBackground($width, $height, $word, $backgroundType, $backgroundImages = [], $morphBackground = true, $blurBackground = true)
    {
            ...
            switch ($backgroundType) {
                ...
                case self::BACKGROUND_TYPE_WHITE_WITH_SQUIGGLES:
                    ...
                    for ($i = 0; $i < strlen($word)+1; $i++) {
                        ...
                        imagepolygon($tempBackground, $points, intval(count($points)/2), $textColor2);
                    }
                    break;
                case self::BACKGROUND_TYPE_MORPHED_IMAGE_BLOCKS:
                    ...
                    for ($i = 0; $i < $width*2; $i += $blocksize) {
                        ...
                        for ($j = 0; $j < $height*2; $j += $blocksize) {
                            $imageIndex = RandomContentUtility::getRandomNumberInRange(0, count($tempImages)-1);
